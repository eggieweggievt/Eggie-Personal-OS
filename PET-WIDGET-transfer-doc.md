...
> **Eggie's sheet (`pet-sprite.png`) is a uniform 96×96 grid, 7 columns × 5 rows (672×480)**, per its Stream Avatars export (`pet-stream-avatar-data.json`). Rows top-to-bottom: 0 idle, 1 walk, 2 sit, 3 stand, 4 jump. So `walk` = the 7 frames of row 1 `{x:0..576 step 96, y:96, w:96, h:96}` and `sit` = `{x:0,y:192,w:96,h:96}`. If the deployed PNG isn't exactly 672×480, run `PetWidget.measure("pet-sprite.png")` and adjust.
...
> **In Eggie's build** it's wired to the existing `analyze` Edge Function's `ask` mode via the `aiCall("ask",{question})` helper, so the pet answers from the same OS brain as the Ask tab. In demo mode it returns a friendly "deploy the function" note.
...
2. **Crisp, not blurry:** keep `image-rendering: pixelated` (it's on by default; pass `smooth:true` only if your art is high-res and you want bilinear scaling). Pick a `scale` that looks good (2–2.4 for ~35px sprites; ~1.0–1.2 for ~96px sprites).
...
6. **Reduced-motion / calm mode → sit still** (no walking, no throw idle jitter). In Eggie's build, `reduceMotion()` also checks `body.calm`.
